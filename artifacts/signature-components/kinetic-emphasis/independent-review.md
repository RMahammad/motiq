...
| 16 | Showcase quality | **9** | Live preview dominates the page, every control is real, install command is brand-correct (`motiq.dev/r/kinetic-emphasis.json`, no `@scope` placeholder), stage has working light/dark + reduce-motion toggles, two scales demonstrated. |
