...
The project is operating as a **limited private preview** (full record: [docs/45 §Private-preview phase decision](45-paid-launch-decision-gate.md)). This does **not** move any row below to *Approved*: provider selection, all pricing, licensing, refund, team/agency, and update-duration decisions remain **Open / Deferred until paid-launch preparation**. Checkout and pricing stay disabled; Batch 6+ development proceeds on the rapid-release track. Preview terms require owner approval via `MOTIONSTACK_PREVIEW_TERMS_APPROVED=1` (not set).
