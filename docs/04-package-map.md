...
Purpose: marketing sections (Hero, FeatureGrid, CTA, …) composed from the primitives/components. Accepts content via props/slots — **no hard-coded marketing copy**. Adds no new engine. See [`animated-section-authoring`](../.claude/skills/component-authoring/SKILL.md).
