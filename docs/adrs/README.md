...
**Status as of the 2026-07-14 architecture spike:** `0001` (monorepo), `0006` (bundler), and `0007` (package format) are now **Accepted** — the spike built `@scope/{tokens,motion,react}` with tsdown, proved `"use client"` survives, shipped three `publint`-clean primitives (Reveal/InView/Stagger) with 27 passing tests + size-limit budgets, and built the Next 16 App Router + Vite + packed-tarball fixtures. The rest remain **Proposed** pending their own validation.
