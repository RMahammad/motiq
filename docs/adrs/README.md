...
**Status as of the 2026-07-14 architecture spike:** `0001` (monorepo) and `0006` (bundler) are now **Accepted** — the spike built `@scope/{tokens,motion,react}` with tsdown, proved `"use client"` survives, and built the Next 16 App Router + Vite fixtures. The rest remain **Proposed** pending their own validation.
...
| [0007](0007-package-format.md) | Package format (ESM-only, subpath exports, preserve directives) | Proposed | [`03`](../03-architecture.md) | no |
