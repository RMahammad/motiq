...
export function ChartRecorderPreview() {
  ...
  return (
    <div className="flex w-full max-w-[960px] flex-col gap-4">
        ...
        >
          ...
          <div className="relative flex min-h-[440px] flex-col justify-center px-7 py-10 sm:px-10">
            ...
            <p className="mt-4 max-w-[42ch] text-[15px] leading-relaxed" style={heroBodyStyle}>
              A slow recency sweep writes each lane, fading into a settled trace behind it - so the
      ...
      <ControlBar label="Background controls">
        <span className="text-[12.5px] font-medium text-[var(--color-muted)]">Lanes</span>
        <ControlSegmented
          label="Instrument lanes"
          value={String(lanes)}
          onChange={(v) => setLanes(Number(v))}
          options={[
            { value: "2", label: "2" },
            { value: "3", label: "3" },
            { value: "4", label: "4" },
...
export default ChartRecorderPreview;
