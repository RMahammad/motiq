...
//   2. The env var MOTIQ_ADMIN_SECRET MUST be set. If it is not, every request
//      403s "admin-disabled" — the admin surface is OFF unless the owner enables it.
//   3. Every request must carry `x-motiq-admin-secret` header matching that
...
function adminGuard(req: Request): NextResponse | null {
  ...
  const secret = process.env.MOTIQ_ADMIN_SECRET;
  if (!secret) {
    return NextResponse.json(
      { error: "admin-disabled", hint: "set MOTIQ_ADMIN_SECRET to enable preview-admin" },
      { status: 403 },
    );
  }
  // 3. Header must match.
  const provided = req.headers.get("x-motiq-admin-secret") ?? "";
