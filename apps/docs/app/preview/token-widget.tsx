...
// Shared create / rotate / revoke registry-token widget for the PREVIEW
// onboarding + dashboard pages. Calls the gated /api/preview/tokens route.
//
// A freshly-created or rotated plaintext token is shown EXACTLY ONCE here in the
// browser — it is never stored and never returned by a listing, so the customer
// must copy it now. `tokens` (metadata only, no plaintext) is passed from the
// Server Component and refreshed via router.refresh(). The list only ever shows a
// short prefix; the full token is never rendered after issuance.
    ...
    <div className="flex flex-col gap-5">
      ...
      >
        <div className="flex flex-1 flex-col gap-1.5">
          <label htmlFor="preview-token-label" className="text-[12.5px] font-medium text-[var(--color-fg)]">
            Token label
          </label>
          <input
            id="preview-token-label"
            type="text"
            value={label}
        ...
        <div className="flex flex-col gap-1.5">
          <label htmlFor="preview-token-env" className="text-[12.5px] font-medium text-[var(--color-fg)]">
            Environment
          </label>
          <SelectControl
            id="preview-token-env"
            value={environment}
      ...
      {active.length === 0 && revoked.length === 0 ? (
        <p className="text-[13px] text-[var(--color-muted)]">
          No registry tokens yet. Create one above to install your Pro preview source.
        </p>
      ) : (
              ...
              >
                <div className="min-w-0">
                  ...
                  <p className="mt-1 truncate font-mono text-[12px] text-[var(--color-muted)]">
                    ...
                    {t.expiresAt != null ? ` · expires ${fmtDate(t.expiresAt)}` : ""}
