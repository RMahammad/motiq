...
import { LoomDraft } from "./loom-draft";
...
describe("LoomDraft", () => {
  ...
  it("renders a canvas inside an aria-hidden decorative layer", () => {
    const { container } = render(<LoomDraft seed={3} />);
  ...
  it("flips data-motion to static under the reducedMotion prop", () => {
    const { container: animated } = render(<LoomDraft seed={7} />);
    expect(bg(animated)?.getAttribute("data-motion")).toBe("animated");
    cleanup();
    const { container: still } = render(<LoomDraft seed={7} reducedMotion />);
  ...
  it("does not throw when the 2d context is unavailable (jsdom)", () => {
    ...
    expect(() => {
      const { unmount } = render(<LoomDraft seed={9} />);
      unmount();
    }).not.toThrow();
  });
  ...
  it("mounts and unmounts cleanly under reduced motion (static frame path)", () => {
    expect(() => {
      const { unmount } = render(<LoomDraft seed={2} reducedMotion />);
      unmount();
    }).not.toThrow();
  });
  ...
  it("mounts and unmounts cleanly with speed=0 (stalled, no loop)", () => {
    expect(() => {
      const { unmount } = render(<LoomDraft seed={2} speed={0} />);
      unmount();
    }).not.toThrow();
  });
  ...
  it("does not schedule a requestAnimationFrame loop under reduced motion", () => {
    ...
    const { unmount } = render(<LoomDraft seed={6} reducedMotion />);
