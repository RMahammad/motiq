 ...
 *   MOTIQ_ENABLE_DEV_ADMIN=1  (and launchMode NOT launched/public-beta).
...
async function main() {
  ...
  if (ping.status !== 200) {
    record("dev-admin enabled (MOTIQ_ENABLE_DEV_ADMIN=1)", false, `reset returned ${ping.status} ${JSON.stringify(ping.json)}`);
    return finish();
  }
